...
typedef CmapData {
    directory: Array<CmapEntry>,
    subTables: Array<CmapSubTable>
}

@:forward
abstract CmapTable( arrCmapSubTable: Array<CmapSubTable> ) to Array<CmapSubTable> { 
    public
    ...
    function read( bytes: Bytes ): CmapTable {
        if( bytes == null ) throw 'no cmap table found';
        var input           = new BytesInput( bytes );
        input.bigEndian     = true;
        var version         = input.readUInt16();
        var numberSubtables = input.readUInt16();
        var directory       = new Array<CmapEntry>();
        for( i in 0...numberSubtables ){
            directory.push( { platformId:         input.readUInt16()
                            , platformSpecificId: input.readUInt16()
                            , offset:             input.readInt32()    } );
        }
        var subTables:= new Array<CmapSubTable>();
        for( i in 0...numberSubtables ){
